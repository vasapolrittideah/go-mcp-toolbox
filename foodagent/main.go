...
const systemPrompt = `
You're a helpful food ordering assistant. You handle searching for
restaurants and products, checking whether a restaurant is open or closed,
listing a restaurant's menu, adding new products, and updating remaining
stock quantities. When you mention a restaurant, include its name and id.
When you mention a product, include its name, id, price, and remaining
stock quantity. These ids are very important for any follow-up operations,
so always surface them. Don't ask for confirmations from the user.
`
...
var queries = []string{
	"ร้านไหนเปิดอยู่บ้างตอนนี้?",
	"ร้านก๋วยเตี๋ยวเรือลุงหมูมีเมนูอะไรขายบ้าง?",
	"ลูกชิ้นเนื้อพิเศษของร้านนั้นสต็อกหมดแล้ว ช่วยเติมสต็อกให้เป็น 20 ชิ้นหน่อย",
	"ช่วยหาร้านที่มีคำว่า 'พิซซ่า' ในชื่อร้าน",
	"ร้านพิซซ่าอิตาเลียนโน่ปิดอยู่ ช่วยเปิดร้านให้หน่อย",
}
...
func main() {
	...
	tools, err := toolboxClient.LoadToolset("food-toolset", ctx)
	if err != nil {
		log.Fatalf("Failed to load tools: %v\nMake sure your Toolbox server is running with food-tools.yaml and the toolset is configured.", err)
	}
